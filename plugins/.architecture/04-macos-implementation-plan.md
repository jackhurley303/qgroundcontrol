...
- **Run settings per unit:** most units are Sonnet-fit with a `code-reviewer` agent dispatch (sonnet) as the review gate (mechanical moves, CMake, QML page work: U1.4–U1.6, U2.5, U2.7, U3.2). Use **Opus** for the ABI-sensitive design units (U2.1–U2.4 service/vtable decisions), the loader state machine (U1.2/U1.3), and trust logic (U3.3/U3.4) — with the `code-reviewer` gate at opus for those. Units run via `/implement-unit` (one fresh chat each).

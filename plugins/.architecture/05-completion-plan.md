...
**Next: U4.3** — expose the active replay link host-side (Q3's second prerequisite) — **Sonnet / medium / thinking on**:

```
/implement-unit plugins/.architecture/05-completion-plan.md U4.3
```

**Then — Stage 4 resumes in the [qdrive plan](../qdrive/docs/sdk-burndown-migration.md)** — Q3 (unblocked by U4.1 + U4.3), then Q4–Q6; that doc's own Next carries their commands and run settings. Stage 4 is planned and executed there, not here — this doc's row ticks when that checklist completes.
...
### U4.3 — Host UI sources the replay link itself (`LinkManager::activeLogReplayLink`) — Q3's second prerequisite
- **Scope:** break the host status bar's dependency on the plugin-supplied raw link pointer (discovered by Q3 grounding 2026-07-18 — see Status). `LinkManager` gains `Q_PROPERTY(LogReplayLink* activeLogReplayLink READ activeLogReplayLink NOTIFY activeLogReplayLinkChanged)` backed by a `QPointer<LogReplayLink>` member: set in the `TypeLogReplay` case of `createConnectedLink()` (the single creation point, right where U4.1's `ReplaySeekApplier` attaches), cleared when that link emits `disconnected` (guard against a stale clear if a newer link replaced it; `Q_MOC_INCLUDE("LogReplayLink.h")` per the `LogReplayLinkController` precedent). `LogReplayStatusBar.qml` then binds declaratively — `link: QGroundControl.linkManager.activeLogReplayLink` on its `LogReplayLinkController` — and **deletes** the `Connections { onIsActiveChanged: controller.link = _replay.logReplayLink }` handoff block and both imperative `controller.link =` assignments (`loadLogFile()`, the file-dialog `onAcceptedForLoad`), which would otherwise destroy the binding; those sites just call `startLogReplay()` and let the property propagate. Covers **every** initiator uniformly — host-initiated, plugin-driven via `qgc.replay/1`, and today's internal-tier qdrive (which also routes through `LinkManager::startLogReplay`) — the same plugin-agnostic-host-improvement story as U4.1. `QGCReplayExtension.h` gets a doc-comment note only (vtable frozen): hosts source the link from their own state; implementations may return nullptr from `logReplayLink()`.
...
- **Depends on:** nothing outstanding here. The qdrive plan's Q3 depends on this unit.
- **Done means:** build green; live verify — host-initiated replay (Load Telemetry Log button + `openTelemetryLog`) and a qdrive flight replay both drive a fully working status bar (play/pause/slider/times) with the `Connections` handoff gone; a unit test only if grounding finds a clean seam (exercising the property needs `LinkManager::startLogReplay` + a scratch tlog and async link teardown — if it's fiddly, record it as live-verified instead, the property logic is ~6 lines); `code-reviewer` (sonnet) clean.
...
- [ ] **U4.3** — Host UI sources the replay link itself (`LinkManager::activeLogReplayLink`; Q3's second prerequisite — added 2026-07-18 after Q3 grounding found the status-bar handoff gap)

...
// Floating plugin panel for the plan view.
...
Item {
    ...
    // Exposed so PlanView can reparent the Loader when popping out to a window.
    property alias contentLoader: _contentLoader
    property alias contentArea:   _contentColumnLayout

    visible: expanded || _panel.opacity > 0
            ...
            Loader {
                ...
                onLoaded: {
                    ...
                    if (item.hasOwnProperty("planMasterControllerPlanView"))
                        item.planMasterControllerPlanView = _root.planMasterController
                }
            }
        }

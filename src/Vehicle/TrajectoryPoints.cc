...
void TrajectoryPoints::bulkLoad(const QList<QGeoCoordinate>& coords)
{
    ...
    for (const QGeoCoordinate& coord : coords) {
        if (_lastPoint.isValid()) {
            // Same 3D thinning as _vehicleCoordinateChanged, without the flight distance update
            const double horizontalDistance = _lastPoint.distanceTo(coord);
            double altitudeDelta = coord.altitude() - _lastPoint.altitude();
            if (!std::isfinite(altitudeDelta)) {
                altitudeDelta = 0.0;
            }
            const double distance = std::hypot(horizontalDistance, altitudeDelta);
            if (distance > _distanceTolerance) {
                ...
                double north = 0.0;
                if (horizontalDistance > 0.0) {
                    const double azimuthRad = qDegreesToRadians(_lastPoint.azimuthTo(coord));
                    east = std::sin(azimuthRad) * horizontalDistance / distance;
                    north = std::cos(azimuthRad) * horizontalDistance / distance;
                }
                const QVector3D newDirection(static_cast<float>(east), static_cast<float>(north),
